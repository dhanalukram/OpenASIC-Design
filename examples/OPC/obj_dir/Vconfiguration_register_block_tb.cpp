...
void Vconfiguration_register_block_tb::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vconfiguration_register_block_tb::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
std::unique_ptr<VerilatedTraceConfig> Vconfiguration_register_block_tb::traceConfig() const {
...
void Vconfiguration_register_block_tb___024root__trace_decl_types(VerilatedVcd* tracep);

void Vconfiguration_register_block_tb___024root__trace_init_top(Vconfiguration_register_block_tb___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vconfiguration_register_block_tb___024root__trace_decl_types(tracep);
    Vconfiguration_register_block_tb___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}
...
VL_ATTR_COLD void Vconfiguration_register_block_tb::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vconfiguration_register_block_tb::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vconfiguration_register_block_tb___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
